...
ansible-playbook -i "environments/${ENVIRONMENT}" ${ANSIBLE_PLAYBOOKS[@]} \
  --extra-vars "config_root_dir=$config_root_dir env_hosts_dir=$ENV_DIR inventory_dir=$ENV_DIR docker_host=unix:///var/run/docker.sock docker_python_module=docker docker_registry='' docker_registry_username='' docker_registry_password='' kafka_certs_dir=$KAFKA_CERTS_DIR" -v
