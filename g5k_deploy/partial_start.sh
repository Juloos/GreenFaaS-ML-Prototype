...
for HOST in "${OW_HOSTS[@]}"; do
  echo "  on $HOST"
  ssh $SSHFLAGS root@$HOST "screen -dmS ow ./greenfaas/g5k_deploy/run_openwhisk.sh \|\& ts '[%F %T] ' \>ow.log 2\>\&1 \&" |& sed "s/^/    /"
done
...
for HOST in "${SWIFT_HOSTS[@]}"; do
  echo "  on $HOST"
  ssh $SSHFLAGS root@$HOST "screen -dmS swift ./greenfaas/g5k_deploy/run_swift.sh \|\& ts '[%F %T] ' \>swift.log 2\>\&1 \&" |& sed "s/^/    /"
done
