...
for HOST in "${OW_HOSTS[@]}"; do
  echo "  on $HOST"
  ssh $SSHFLAGS root@$HOST "screen -dm ./greenfaas/g5k_deploy/run_openwhisk.sh \| ts '[%F %T] ' >openwhisk.log 2>&1 &" |& sed "s/^/    /"
done
...
for HOST in "${SWIFT_HOSTS[@]}"; do
  echo "  on $HOST"
  ssh $SSHFLAGS root@$HOST "screen -dm ./greenfaas/g5k_deploy/run_swift.sh \| ts '[%F %T] ' >swift.log 2>&1 &" |& sed "s/^/    /"
done
