...
for HOST in "${OW_HOSTS[@]}"; do
  ...
  ssh -o StrictHostKeyChecking=no root@$HOST "./greenfaas/g5k_deploy/run_text2speech.sh '$(IFS=','; echo "${SWIFT_HOSTS[*]}")' '$ITERATIONS' '$RUNS' >tts.log 2>&1 &" |& sed "s/^/    /"
...
while [ -n "$(tr -d ' ' <<<"${OW_HOSTS[@]}")" ]; do
  for HOST in "${OW_HOSTS[@]}"; do
    ...
    if [[ "$(grep "Done" logs/$HHOSTNAME.$OAR_JOB_ID.log)" ]]; then
      scp -r root@$HOST:/root/greenfaas/energy_results/$HHOSTNAME ./energy_results/${ITERATIONS}i-${RUNS}r-${1}ow_${OAR_JOB_ID} 2>/dev/null
      echo "  on $HOST: done"
